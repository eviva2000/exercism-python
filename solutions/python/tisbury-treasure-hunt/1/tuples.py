...
def compare_records(azara_record, rui_record):
    ...
    first_coordinate=get_coordinate(azara_record)
    converted_coordinate=convert_coordinate(first_coordinate)
    if converted_coordinate==rui_record[-2]:
        return True
    return False
...
def clean_up(combined_record_group):
    ...
    for item in combined_record_group:
       refined_item=item[:1]+item[2:]
       cleaned_records.append(f"{refined_item}\n")
