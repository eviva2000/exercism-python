...
def round_scores(student_scores):
    ...
    for score in student_scores:
       rounded= round(score)
       rounded_score_list.append(rounded) 
...
def count_failed_students(student_scores):
    ...
    for score in student_scores:
      if score<=40:
         list_bellow_40.append(score)

    return len(list_bellow_40)
...
def letter_grades(highest):
    ...
    for number in range(0,4):
        lower_threshold_score=int(41+number*interval)
        lower_threshold_scores.append(lower_threshold_score)
...
def student_ranking(student_scores, student_names):
    ...
    for rank,score in enumerate(student_scores,start=1):
        student=f"{rank}. {student_names[rank-1]}: {score}"
